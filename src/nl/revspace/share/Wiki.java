...
public class Wiki implements Serializable
{
	...
	private UploadToRevspaceActivity uplActivity;
     ...
     *  @since 0.02
     */
    public Wiki(UploadToRevspaceActivity uplActivity)
    {
    	this("",uplActivity);
     ...
     *  en.wikipedia.org)
     */
    public Wiki(String domain, UploadToRevspaceActivity uplActivity)
    {
        if (domain == null || domain.length() == 0)
            //domain = "en.wikipedia.org";
        	//domain = "commons.wikimedia.org";
        	domain = "geilesletjes.slut";
        this.domain = domain;
    ...
    protected void initVars() {
        base = schema + "://" + domain + scriptPath + "/index.php?title=";
        apiUrl  = schema + "://" + domain + scriptPath +  "/api.php?";
    ...
    public Boolean setSchema(String s)
    {
    ...
    public String getScriptPath() throws IOException
    {
        ...
        base = schema + "://" + domain + scriptPath + "/index.php?title=";
        query = schema + "://" + domain + scriptPath +  "/api.php?format=xml&";
        return scriptPath;
    }
    ...
    public synchronized void edit(String title, String text, String summary, boolean minor, int section) throws IOException, LoginException
    {
        ...
        try {
			...
			{
			    CRLException ex = new CRLException("Permission denied: page is protected.");
    ...
    public synchronized void delete(String title, String reason) throws IOException, LoginException
    {
        ...
        if (user == null || !user.isAllowedTo("delete"))
			try {
				throw new ClassNotFoundException("Cannot delete: Permission denied");
     ...
     *  @since 0.17
     */
    public void purge(String... titles) throws IOException, ClassNotFoundException
    {
        if (user == null)
            throw new ClassNotFoundException("You need to be logged in to purge pages via the API.");
        StringBuilder url = new StringBuilder(query);
        ...
        boolean movesubpages) throws IOException, LoginException
    {
        ...
        {
            ClassNotFoundException ex = new ClassNotFoundException("Permission denied: cannot move pages.");
    ...
    public synchronized void rollback(Revision revision, boolean bot, String reason) throws IOException, LoginException
    {
        ...
        if (user == null || !user.isAllowedTo("rollback"))
			try {
				throw new ClassNotFoundException("Permission denied: cannot rollback.");
			} catch (ClassNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
        ...
        if (cookies2.isEmpty())
        {
            ...
            logout();
            try {
				throw new ClassNotFoundException("Cookies have expired.");
			} catch (ClassNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
     ...
     *  @since 0.21
     */
    public synchronized void uploadAndroid(byte[] data, 
    		                        String filename, 
    		                        String contents, 
    		                        String reason) throws IOException, LoginException
    {
        ...
        // upload the image
        log(Level.INFO, "connection set", "upload");
        ...
        //String url = apiUrl + 
        //		     "action=upload&filename="+filename2+
        //		     "&file="+file_contents_here+
        //		     "&token="+encodedtoken     		    ;
        log(Level.INFO, "query string "+url, "upload");
        logurl(url, "upload");
        ...
        log(Level.INFO, "send token urlencoded as " + URLEncoder.encode(wpEditToken, "UTF-8")+"%2B%5C", "upload");
        ...
        // format
        out.writeBytes("Content-Disposition: form-data; name=\"format\"\r\n");
        out.writeBytes("Content-Type: text/plain\r\n\r\n");
        out.writeBytes("xml");
        out.writeBytes("\r\n");
        out.writeBytes(boundary);
        ...
        out.writeBytes(URLEncoder.encode(filename, "UTF-8"));
        out.writeBytes("\"\r\n");
        ...
        log(Level.INFO, "out defined and flushed", "upload");
        ...
        try
        {
            ...
            uplActivity.setEndingMessage("Upload to RevSpace Wiki done!");
    ...
    public synchronized void emailUser(User user, String message, String subject, boolean emailme) throws IOException, LoginException
    {
        ...
        if (this.user == null || !this.user.isAllowedTo("sendemail"))
			try {
				throw new ClassNotFoundException("Permission denied: cannot email.");
			} catch (ClassNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
        ...
        if (cookies2.isEmpty())
        {
            ...
            logout();
            try {
				throw new CRLException("Cookies have expired.");
			} catch (CRLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
     ...
     *  @since 0.18
     */
    public void watch(String title) throws IOException, ClassNotFoundException
    {
        /*
     ...
     *  @since 0.18
     */
    public void unwatch(String title) throws IOException, ClassNotFoundException
    {
     ...
     *  @since 0.18
     */
    protected void watchInternal(String title, boolean unwatch) throws IOException, ClassNotFoundException
    {
     ...
     *  @since 0.18
     */
    public String[] getRawWatchlist() throws IOException, ClassNotFoundException
    {
     ...
     *  @since 0.18
     */
    public String[] getRawWatchlist(boolean cache) throws IOException, ClassNotFoundException
    {
        // filter anons
        if (user == null)
            throw new ClassNotFoundException("The watchlist is available for registered users only.");
     ...
     *  @since 0.18
     */
    public boolean isWatched(String title) throws IOException, ClassNotFoundException
    {
    ...
    public class User implements Cloneable
    {
        ...
        {
        	log(Level.INFO, "get user info ", "getUserInfo");
    ...
    protected void checkErrors(String line, String caller) throws IOException, LoginException
    {
        ...
        if (line.contains("error code=\"cascadeprotected\""))
        {
            ...
            try {
				throw new AndroidException("Page is cascade protected");
			} catch (AndroidException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
     ...
     *  @since 0.10
     */
    private boolean checkRights(int level, boolean move) throws IOException, CRLException
    {
     ...
     *  @since 0.11
     */
    protected void statusCheck() throws IOException, CRLException
    {
        // @revised 0.18 was assertions(), put some more stuff in here
    	log(Level.INFO, "status check ", "statusCheck");
